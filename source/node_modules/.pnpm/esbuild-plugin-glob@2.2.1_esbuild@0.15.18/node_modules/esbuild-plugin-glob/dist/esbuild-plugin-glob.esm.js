    ...
    async setup(build) {
      ...
      if (build.initialOptions.watch) {
        ...
        const handleBuildResult = async (entry, buildResult) => {
          ...
          const inputs = Object.values(buildResult.metafile.outputs).filter((output) => !!output.entryPoint).flatMap(
            (output) => Object.keys(output.inputs).filter((input) => !input.includes("node_modules")).map((input) => normalizePath(input))
          );
          watcher.add(inputs);
